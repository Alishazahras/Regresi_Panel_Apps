...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "Analisis Panel"),
                    ...
                    navlistPanel(
                      ...
                      tabPanel("Pneumonia: Definisi & Berita",
                               ...
                               div(class = "teori-box",
                                   ...
                                   tags$ul(
                                     tags$li("Persentase penduduk usia 35–44 tahun yang merokok."),
                                     tags$li("Persentase rumah tangga yang memiliki akses terhadap sanitasi layak."),
                                     tags$li("Persentase bayi kurang dari 6 bulan yang mendapatkan ASI eksklusif."),
                                     tags$li("Persentase pemberian vitamin A pada balita."),
                                     tags$li("Persentase Bayi Berat Lahir Rendah (BBLR)."),
                                     tags$li("Persentase bayi gizi rendah (BB/U)."),
...
server <- function(input, output) {
  ...
  output$viz_output_ui <- renderUI({
    ...
    if(input$viz_type == "Tren per Wilayah") {
      ...
      plotOutput("plot_hist", height = "500px")
    }
  })
  ...
  output$plot_trend <- renderPlot({
    ...
    ggplot(df, aes(x = Time, y = Y, group = ID)) +
      geom_line(color = "blue", size=1) + geom_point(size=2) +
  ...
  output$plot_corr <- renderPlot({
    ...
    df <- data_clean()
    cor_long <- df %>% group_by(across(all_of(input$var_time))) %>%
      do({
        d <- select(., all_of(c(input$var_y, input$var_x)))
        if(nrow(d)>2) melt(cor(d, use="complete.obs")) %>% mutate(Tahun = unique(.[[input$var_time]])) else data.frame()
      }) %>% ungroup()
    req(nrow(cor_long)>0)
    ggplot(cor_long, aes(Var1, Var2, fill=value)) + geom_tile() +
      geom_text(aes(label=round(value,2)), color="white") +
      scale_fill_viridis(option="C") + facet_wrap(~Tahun) + theme_minimal()
  })
  ...
  output$plot_hist <- renderPlot({
    ...
    ggplot(df, aes(x = .data[[input$var_y]])) +
      geom_histogram(bins = 20, fill = "#69b3a2", color = "black") +
      theme_bw() +
      labs(
        title = paste("Distribusi", input$var_y, "Keseluruhan"),
        x = paste("Jumlah Kasus", input$var_y),
        y = "Frekuensi"
      )
  })  
